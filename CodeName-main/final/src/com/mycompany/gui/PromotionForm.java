...
public class PromotionForm extends BaseForm{
    ...
    public PromotionForm(Form previous,Resources res) {
...
*/
     ...
     final String[] image_name = {""};
        ...
        {
            if((pourcentage.getText().length()==0))
                Dialog.show("Alert", "Please fill all the fields " , new Command ("Ok")); 
            else {
            
            try{
            Promotion p = new Promotion (pourcentage.getAsInt(BASELINE));
            
            if(new ServicePromotion().add(p)){
                Dialog.show("Success", "Connection accepted", new Command("ok")); 
                new ListPromotionForm(previous,res).showBack();
            }
            else 
